...
Numbered guarantees: [invariants.md](invariants.md) 40–47 and 49 (reactions).
...
`deleted_storage`, `pending_wipe_cleanup`, and `pending_scrub`. Schema migrations are serialized under the SQLite writer lock.
...
trying to remove it; an access failure retains durable scrub work and the file for retry. The next
migration (the reactions table, schema v7 on this branch — the number is assigned at merge, see
decisions/00164) adds `reactions`, described under "Reactions" in §6.
