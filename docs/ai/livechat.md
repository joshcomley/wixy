...
## 17. View-once media and spotlight reveal (round 2 item 13, `spec/server-chat/06-view-once-media.md`)
...
- `messages.view_spotlight`: `INTEGER NOT NULL DEFAULT 0` (`CHECK(view_spotlight IN (0, 1))`) — photo only.
...
- `POST /messages/{seq}/view-once/open`: Atomically claims the view. Sender cannot open own message (403 `own_message`). First claimant wins (200 with `{durationS, spotlight, kind, mime}`); subsequent claims return 409 `already_opened`.
