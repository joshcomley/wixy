...
never close an open drawer — only genuine route changes do, decisions/00079); `router.ts` (hash routes: pages/edit/theme/
...
explainer — decisions/00079); `historyPanel.ts`
...
dump, decisions/00079); `mediaPanel.ts` + `mediaDialog.ts` (library + picker);
