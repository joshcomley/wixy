...
| POST | `server/messages/view-once` | `send_view_once_message` | `{"clientId":str(8-64),"sender":str(1-32,trimmed),"deviceId":str(8-64),"attachmentId":hex32,"durationS":2\|5\|30\|null,"tease":bool,"spotlight":bool?,"replyToSeq":int(≥1)?}` — exactly one attachment (photo or video), no text; `tease` permitted only for photos; `spotlight` is a TRANSITIONAL alias of `tease` for tabs loaded before the rename (either key switches Tease on; remove after 2026-10-27, decisions/00172); 422 `not_ready` if attachment is not ready | 201 `{"message":<Message>}` (200 on replayed `clientId`); 422 `{"error":"invalid","detail":str}` or `{"error":"not_ready"}` |
| POST | `server/messages/{seq}/view-once/open` | `open_view_once` | `{"claimId":hex32,"sender":str(1-32,trimmed)}` — claims the single view; sender cannot claim own message (403 `own_message`) | 200 `{"durationS":int\|null,"tease":bool,"spotlight":bool,"kind":"photo"\|"video","mime":str}` (`spotlight` = transitional alias of `tease`, remove after 2026-10-27); 403 `{"error":"own_message"}`; 404 `{"error":"not_found"}`; 409 `{"error":"already_opened"}`; 422 `{"error":"invalid","detail":str}` |
...
| GET | `server/push/config` | `get_push_config` | — (token) | 200 `{"enabled":bool,"publicKey":str\|null}`; 401 locked |
