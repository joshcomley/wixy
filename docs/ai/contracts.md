...
| DELETE | `server/messages/{seq}` | `delete_message` | — | 204 when deleted and scrubbed, or 202 `{"scrubPending":true}` when durable background scrubbing remains; idempotent even when the message is already gone |
| POST | `server/wipe` | `wipe_chat` | exactly `{"confirm":"WIPE"}` | 204 when deleted and scrubbed, or 202 `{"scrubPending":true}` while durable background scrubbing remains; every other body, including extra keys, is 422 |
| GET | `server/stream?after=` | `stream` | query `after?:int` (event cursor) | **SSE**, see §4 |
| GET | `server/usage` | `usage` | — | `{"usedBytes":int,"quotaBytes":int,"freeBytes":int,"mediaAvailable":bool,"scrubPending":bool}` |
