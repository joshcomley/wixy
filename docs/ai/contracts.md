...
| GET | `global` | `get_global` | — | `{"global": <JsonObject>}` (`content/_global.json`, overlay-merged; an untouched file reads as `{}`, never 404 — decisions/00127) — Settings > Contact reads `phone`/`email`/`address` from here; 503 |
