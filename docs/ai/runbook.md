...
| `indexable` | `false` | gates `robots.txt`/`noindex`/`sitemap.xml` (build-time) plus the live `X-Robots-Tag: noindex` response header on public media/`/api/version*` (request-time, Inv 37); stays `false` permanently for this staging/admin host — the public canonical builds with a CLI `--indexable true` override instead (see "Public site: GitHub Pages custom domain" below) |
