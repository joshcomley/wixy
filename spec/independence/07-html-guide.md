...
- **Track J — Josh does this once** (≈45 min): publish the engine (post-audit click,
  02 §3) · run the pre-cutover backup job · hand over the transfer buttons (GitHub repo
  transfers TO her org once it exists) · lower DNS TTL at the registrar · final DNS
  flip (with her, section 8).
- **Track P — Purdi does this** (each its own chapter, independent sittings):
  1. Password manager setup (if none).
  2. GitHub account + organisation; accept the two repo transfers; fork the engine.
  3. DigitalOcean account + the droplet (exact plan/region screenshots; card step
     flagged with the £~12/mo cost).
  4. Cloudflare account + add her domain + registrar nameserver change (registrar
     screenshots for the likely registrars; "site keeps working during this" note) +
     create the tunnel (copy the token) + the Access app (add her email + Josh's).
  5. Anthropic account + API key + set the monthly budget in HER Anthropic console
     too (belt and braces with 05 §2's app-side cap).
  6. The droplet setup: open DO web console → paste `setup.sh` → answer its prompts
     from the password manager → watch `verify.sh` go green.
...
- **Appendix B — Costs** (~£12/mo DO + AI usage + ~£10/yr domain; everything else free).
