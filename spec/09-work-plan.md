...
|---|---|---|---|
| 1 | WX | **Scaffold**: `pyproject.toml` (py3.14, fastapi/uvicorn/httpx/pillow/bs4+html5lib/jinja2/pytest+xdist/mypy/ruff), `builder/` + `wixy_server/` package skeletons, `admin-ui/` + `editor/` TS workspaces (strict tsconfig, esbuild scripts, vitest), CI (`ci.yml`: ruff+mypy+pytest -n auto+tsc+esbuild+vitest+playwright), repo `CLAUDE.md` (dev commands, layout, spec pointer), `decisions/00001` (architecture, from 01 §7) | 01 |
...
| 3 | CA | **Migration step 1**: move pages under `pages/`, add partial markers, empty-shim partials; capture parity baseline (host it in WX per 03 §5); site CI (`ci.yml` calling WX builder@main: validate+build+parity) replaces nothing yet (deploy.yml stays until #12) | 03 §3.1, §5, §7 |
...
| 12 | CA | **Cutover**: point Wixy's project checkout at CA main, first real publish from the admin, retire GH Pages (`deploy.yml`, `.nojekyll` removed), README updated with new home | 03 §3.5, 07 |
...
- Infra actions in #11 that need elevation or CF credentials follow 07's runbook exactly
  (admin gate for service-level ops; CF API token from Biosphere storage for DNS/Access).
