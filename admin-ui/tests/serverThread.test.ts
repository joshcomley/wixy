...
describe("reply to a message (round 2 ruling item 10)", () => {
  ...
  it("a full-size composer button (not a per-chip badge) targets the most recently staged photo/video and retargeting clears the previous flag (Item 2, redesigned after operator report: the badge's picker was clipped invisible)", async () => {
    getHistory.mockResolvedValue(emptyHistory());
    uploadServerAttachment
      .mockResolvedValueOnce({
    ...
    expect(viewOnceButton.hidden).toBe(true);
    ...
    sheetA.querySelector<HTMLButtonElement>(".wx-srv-view-once-dur-btn:nth-child(2)")!.click();
    await flush();
    ...
    // Attaching a second photo retargets the button to it and clears A's flag automatically —
    // there is now only ONE control, so there is nothing left for the user to forget to clear.
    ...
    viewOnceButton.click();
    await flush();
    document.body.querySelector<HTMLButtonElement>(".wx-srv-view-once-dur-btn:nth-child(3)")!.click();
    await flush();
  ...
  it("hides the view-once button once nothing eligible remains staged, and reopening its sheet is a real, unclipped body-level element (operator report, round 2)", async () => {
    getHistory.mockResolvedValue(emptyHistory());
  ...
  it("bounds preparing retry loop to 60s cap and restores draft (Item 5)", async () => {
    ...
    await flush();
    document.body.querySelector<HTMLButtonElement>(".wx-srv-view-once-durations button:nth-child(2)")!.click();
    await flush();
